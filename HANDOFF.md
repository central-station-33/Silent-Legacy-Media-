...
1. **Two GitHub repos existed for this project** — `Silent-Legacy-Media`
   (empty stub) and `Silent-Legacy-Media-` with a trailing dash (this
   repo's actual content). They've been consolidated into one:
   `Silent-Legacy-Media` (no dash) now has everything. The dash-suffixed
   repo is redundant and safe to archive/delete once you've confirmed
   nothing here is missing — that deletion wasn't done automatically
   since it's destructive and needs org-admin action in the GitHub UI.
2. **Retool is retired.** There were actually *two* separate,
   ...
   *old* Retool-managed Postgres, not this new Supabase database.**
   ...
   See `web/README.md` (if present) or the PR description for exact
   Vercel/Supabase env var setup.
