...
`silent_legacy_stories`, `silent_legacy_raw_items`). The app connects
with the `service_role` key (server-only, read in `lib/supabase.ts`,
which is marked `server-only` so it can't end up in a client bundle) —
RLS denies the `anon` role on every table, so even a leaked anon/publishable
key can't read or write anything. Access control lives entirely in the
app: the password gate in `middleware.ts`. (Earlier versions of this app
used the anon key with permissive RLS policies -- that's been retired;
see `HANDOFF.md` for the migration.) A further hardening step, not done
yet, would be real Supabase Auth scoped to `authenticated` instead of a
...
- `SUPABASE_URL`, `SUPABASE_SERVICE_ROLE_KEY` — server-only, from the
  Supabase project's API settings (the `service_role` secret, not the
  anon/publishable key — see "Data" above for why).
